...
# --- FUNÇÃO DE LEITURA (Lê 0 a 1023 e publica em Radianos) ---
    def read_motors_callback(self):
        ...
        for joint_name, dxl_id in self.joint_map.items():
            try:
                # Adicionamos a captura do erro de hardware (dxl_error)
                ...
                if dxl_comm_result == COMM_SUCCESS:
                    # Se o motor mandar aviso de hardware (ex: superaquecimento), o nó avisa em vez de quebrar
                    if dxl_error != 0:
                        self.get_logger().warn(f"Atenção: Motor {dxl_id} reportou erro interno (Código: {dxl_error}). Verifique o LED vermelho.")
                ...
                # O SDK do Dynamixel dá esse erro se o motor mandar um pacote corrompido ou vazio.
                # Agora nós capturamos o erro e pulamos a leitura desse motor sem travar o programa.
