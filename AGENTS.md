...
O repositório inteiro é um mini-workspace: `src/ax12_control/` e `src/adam_description/` são
2 pacotes ROS lado a lado. Buildar a partir da raiz do repositório funciona normalmente — o
...
- Nomes de junta seguem o URDF do pacote `adam_description`: `{lado}_{movimento}_{segmento}_{N}` (ex.:
  `pd_picht_tornozelo_3`). Desde o remapeamento mais recente, o `N` **é** o ID físico do motor
  no barramento (os motores foram regravados pra isso) — confirme sempre contra `joint_map`
  (`src/ax12_control/ax12_control/ax12_controller.py`), que é a fonte de verdade.

## Mapa do repositório

Este repositório é um mini-workspace com 2 pacotes ROS lado a lado em `src/`, seguindo a
convenção do ecossistema ROS 2 (`_description` para o modelo):
...
- `docs/` — documentação: `install.md` (tutorial), `troubleshooting.md` (guia),
  `ref/` (referência), `adr.md` (explicação/diário de bordo), `cabeca.md` (nó da cabeça).
  Documenta principalmente o pacote `ax12_control`; `adam_description` ainda não tem docs
  próprias aqui.

`src/adam_bringup/` (runtime do `ros2_control`, digital twin mock) foi removido — não subia
nada que o hardware real ou o `docs/install.md` dependam. `gait_bridge.py` ficou órfão como
consequência (continua rodando, só que sem nenhum `JointTrajectoryController` ouvindo).
