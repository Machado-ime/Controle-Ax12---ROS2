    ...
    robot_description = {
        'robot_description': Command([
            ...
            PathJoinSubstitution(
                [FindPackageShare('adam_description'), 'urdf', 'adam.urdf.xacro']),
            ' use_mock_hardware:=', use_mock,
        ])
    }

    controllers = PathJoinSubstitution(
        [FindPackageShare('adam_bringup'), 'config', 'ros2_controllers.yaml'])
    rviz_config = PathJoinSubstitution(
        [FindPackageShare('adam_description'), 'config', 'adam.rviz'])
